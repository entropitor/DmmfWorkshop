...
type PersonalName =
    { // A record type used for modeling "AND" relations
      FirstName: Text
      LastName: Text }
...
type OrderLine = { ProductId: ProductId; Qty: OrderQty }
...
type CardInfo = { CardNumber: string }
...
type OrderPlaced = { Method: PaymentMethod; Order: Order }

// Use Function types for workflows
type PlaceOrder = Order -> OrderPlaced
...
//TODO Write some code to make this compile
let emailAddress = Email "test@example.com"
...
let printOrderId (OrderId value) = printfn "OrderId = %i" value
...
let addTwoNumbers: AddTwoNumbers = fun n1 n2 -> n1 + n2
...
let placeOrder: PlaceOrder =
    fun input ->
        ...
        let output = { Order = input; Method = Card { CardNumber = "555" } }
        output
